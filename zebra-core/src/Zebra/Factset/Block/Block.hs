...
module Zebra.Factset.Block.Block (
    Block(..)
...
import           Zebra.Factset.Fact (Fact(..), FactConversionError)
...
data FactError =
  ...
  | FactLogicalSchemaError !LogicalSchemaError
...
factsOfBlock block = do
  ...
  values0 <- first FactStripedError $ Boxed.mapM Striped.toLogical tables
  values <- first FactLogicalSchemaError $ traverse Logical.takeArray values0
...
data ValueState =
  ValueState {
    ...
    , _stateValues :: Boxed.Vector (Boxed.Vector Logical.Value)
    }
...
mkFact :: EntityHash -> EntityId -> AttributeId -> BlockIndex -> Logical.Value -> Fact
...
takeValues :: AttributeId -> Int -> EitherT FactError (State ValueState) (Boxed.Vector Logical.Value)
